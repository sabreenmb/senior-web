      ...
      <table class="table table-bordered">
        <tbody>
          <tr>
            ...
            <td>{{ Form::text('op_name', $id ? $opportunity['op_name'] : null, ['class' => 'form-control',
              'autocomplete' => 'off']) }}</td>
          </tr>
          <tr>
            <td>التاريخ</td>
            <td>{{ Form::text('op_date', $id ? $opportunity['op_date'] : null, ['class' => 'form-control', 'id' =>
              'datePicker', 'autocomplete' => 'off', 'placeholder' => 'حدد التاريخ']) }}
            </td>
          </tr>
          <tr>
            <td>الوقت</td>
            <td>{{ Form::text('op_time', $id ? $opportunity['op_time'] : null, ['class' => 'form-control', 'id' =>
              'timePicker', 'autocomplete' => 'off', 'placeholder' => '24:00']) }}</td>
          </tr>
          <tr>
            <td>الموقع</td>
            <td>{{ Form::text('op_location', $id ? $opportunity['op_location'] : null, ['class' => 'form-control',
              'autocomplete' => 'off']) }}</td>
          </tr>
          <tr>
            <td>عدد المتطوعين</td>
            <td>{{ Form::text('op_number', $id ? $opportunity['op_number'] : null, ['class' => 'form-control',
              'autocomplete' => 'off']) }}</td>
          </tr>
          <tr>
            <td>رابط نموذج التسجيل </td>
            <td>{{ Form::text('op_link', $id ? $opportunity['op_link'] : null, ['class' => 'form-control',
              'autocomplete' => 'off']) }}</td>
          </tr>
        </tbody>
      </table>
